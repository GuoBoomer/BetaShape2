	...
	for (int i = 0; i < cloudA->points.size(); i++) {
		...
		while(current_queue.size()!=0){
			...
			if (!existOfIndexs(current, visited)) {
				regionGrow3(current,noise,block,tri,bc,stop_point,current_queue);
...
void GammaShape::regionGrow3(int seed, vector<int>& noise, blocks * block, tri_index * tri, BallCenter * bc, vector<int>& stop_point, vector<int>& current_queue)
{
	if (existOfIndexs(seed, visited))
		return;
	//成为了种子点才算已访问
	...
	for (int a = 0; a < indexs.size() - 1; a++)
		for (int b = a + 1; b < indexs.size(); b++) {
			...
			//判断是否有停驻点, 是否需要保持return
			if (judgeStopPoints2(bc, heart_of_Circumball, ind)) {
				if ((!existOfIndexs(seed, stop_points)) && (!existOfIndexs(seed, stop_point))) {
					stop_point.push_back(seed);
					return;
				}
...
bool GammaShape::judgeStopPoints2(BallCenter *bc, double Circumball[][3], vector<int> ind) {
	//第一个点 或者没找到公共点 返回false 表示不是Stop Points
	if (bc->set1.tir_points.size() == 0 || ind.size() == 0)
		return false;
	double this_rates = 0;
	...
	for(int i=0;i<ind.size();i++){
		if (bc->set1.tir_points.size() <= ind[i]) {
			cout << "judgeStopPoints 越界:" << ind[i] << " " << bc->set1.tir_points.size() << endl;
			continue;
		}
		...
		float d[4];
		PointXYZ tem = bc->set1.tir_points[ind[i]];
		PointXYZ tem2 = bc->set2.tir_points[ind[i]];
		tem = bc->set1.tir_points[ind[i]];
		tem2 = bc->set2.tir_points[ind[i]];
		//离同一个球心的距离
		d[0] = pow(heart1.x - tem.x, 2) +
			pow(heart1.y - tem.y, 2) +
			pow(heart1.z - tem.z, 2);
		...
		this_rates += scale1 / scale2;
		//小于阈值，非停驻点,否则为停驻点
		
	}
	this_rates = this_rates / ind.size();
	
	double small = this_rates > front_rates ? front_rates : this_rates;
	double big = this_rates <= front_rates ? front_rates : this_rates;

	//阈值对比
	if (front_rates == -1) {
		front_rates = this_rates;
		return false;
	}
	else if (small / big > Threshold) {
			front_rates = (this_rates + front_rates) / 2;
			return false;
	}else {
		return true;
	}
